    ...
    if ( s.startsWith("/demande") ) { // TODO : Faire un like
        ...
        if (vart.length < 4) {
            ...
            if(message.author.id === vart[2] || message.author.username === vart[2]) {
                ...
                resultQueryType = selectSQL("SELECT count(id_carte) FROM carte, famille, enfant WHERE carte.type_carte=famille.id_type and carte.famille=enfant.id_enfant and nom_type = \""+vart[3]+"\" and nom_enfant = \""+vart[4]+"\";");
                
                if(1 === resultQueryType) { // ResultQueryType est une variable qui contient le type de carte, il doit correspondre.
